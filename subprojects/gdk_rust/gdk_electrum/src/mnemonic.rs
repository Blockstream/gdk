...
impl Mnemonic {
    ...
    pub fn to_seed(&self, pd_passphrase: Option<&str>) -> Vec<u8> {
    ...
    pub fn from_str(s: &str) -> Result<Mnemonic, Error> {
        ...
        for word in &words {
            if let Ok(idx) = WORDS.binary_search(word) {
                ...
                return Err(Error::Generic("Mnemonic contains an unknown word".to_string()));
            }
        }
        writer.flush().unwrap();
        let l = data.len();
        let (payload, checksum) = data.split_at(
            l - if l > 33 {
                2
            } else {
                1
        ...
        if Self::checksum(payload).as_slice() != checksum {
            return Err(Error::Generic("Checksum failed".to_string()));
        }
    ...
    pub fn extend(&self) -> Result<Mnemonic, Error> {
        if self.0.len() != 12 {
            return Err(Error::Generic(
                "Can only extend mnemonic of 12 words to 24 words".to_string(),
            ));
        }
        let mut data = Vec::new();
        let mut writer = BitStreamWriter::new(&mut data);
