...
) -> Result<RegistryAssets> {
    let (mut assets, from_local_cache) =
...
pub(crate) fn get_icons(
    params: &RefreshAssetsParams,
) -> Result<RegistryIcons> {
    let (mut icons, from_local_cache) =
        fetch::<RegistryIcons>(AssetsOrIcons::Icons, params)?;
