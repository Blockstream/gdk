...
fn check_xpub_consitency(
    ...
    xpub: ExtendedPubKey,
    bip32_account: u32,
) -> Result<(ScriptType, ExtendedPubKey, u32), Error> {
    match xpub.child_number {
        ChildNumber::Hardened {
            index: n,
        } if n == bip32_account => Ok((script_type, xpub, n)),
        _ => Err(Error::UnsupportedDescriptor),
    }
}
    ...
    if let Descriptor::Sh(sh) = desc {
        if let ShInner::Wpkh(wpkh) = sh.as_inner() {
            if let DescriptorPublicKey::XPub(descriptorxkey) = wpkh.as_inner() {
                if let Some((_, p)) = &descriptorxkey.origin {
                    let n = match_key_origin(&p.clone().into(), 49, coin_type)?;
                    return check_xpub_consitency(ScriptType::P2shP2wpkh, descriptorxkey.xkey, n);
                }
            }
        }
        ...
        if let DescriptorPublicKey::XPub(descriptorxkey) = wpkh.as_inner() {
            if let Some((_, p)) = &descriptorxkey.origin {
                ...
                return check_xpub_consitency(ScriptType::P2wpkh, descriptorxkey.xkey, n);
            }
        }
        ...
        if let DescriptorPublicKey::XPub(descriptorxkey) = pkh.as_inner() {
            if let Some((_, p)) = &descriptorxkey.origin {
                ...
                return check_xpub_consitency(ScriptType::P2pkh, descriptorxkey.xkey, n);
            }
        }
    }
