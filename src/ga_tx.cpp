...
namespace green {
        ...
        session_impl& session, const std::vector<nlohmann::json>& utxos, size_t index, uint32_t sighash_flags) const
    {
        ...
        // Initialization of cache and signing data on the first use
        if (!m_sighash_cache) {
            // TODO: Use a wally constant for the cache size when available
            m_sighash_cache = map_init(16);
        }
        if (!m_scripts && !m_assets && !m_values) {
            std::tie(m_scripts, m_assets, m_values) = get_signing_data(session, utxos);
        }

        // In case of non-taproot sighash, we need to provide the prevout script
