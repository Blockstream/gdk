...
namespace sdk {
    ...
    namespace {
        ...
        void unblind_utxo(nlohmann::json& utxo, const std::string& policy_asset, signer& signer, ga_session& session)
        {
            ...
            if (boost::conversion::try_lexical_convert(json_get_value(utxo, "value"), value)) {
                ...
                try {
                    unblind_t unblinded;
                    if (signer.get_hw_device().is_null()) {
                ...
                if (external) {
                    ...
                    if (!policy_asset.empty()) {
                        if (json_get_value(utxo, "is_relevant", true)) {
                            unblind_utxo(utxo, policy_asset, signer, session);
                        }
                    } else {
                        amount::value_type value;
                        GDK_RUNTIME_ASSERT(
        ...
        const std::string& pubkey, const std::string& script)
    {
        ...
        std::copy(script_bytes.begin(), script_bytes.end(), std::back_inserter(data));
