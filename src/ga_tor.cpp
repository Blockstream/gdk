...
namespace green {
    ...
    static std::map<std::string, std::string> parse_tor_reply_mapping(const std::string& s)
    {
        ...
        while (cursor < source.size()) {
            std::string_view key;
            size_t idx = source.find_first_of("= ", cursor);
            key = source.substr(0, idx);
            if (idx == std::string_view::npos) { // unexpected end of line
                return {};
            }
            cursor = idx;
            ...
            if (cursor < source.size() && source[cursor] == '"') { // Quoted string
                ...
                size_t value_end = source.find(' ', cursor);
                value = source.substr(cursor, value_end - cursor);
                cursor = value_end;
            }
