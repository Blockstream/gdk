...
namespace green {
    ...
    void create_redeposit_transaction_call::initialize()
    {
        ...
        for (auto& asset : utxos.items()) {
            ...
            // exception that we allow fees to be paid from another subaccount
            ...
            for (const auto& utxo : asset.value()) {
                auto& check_subaccount = is_fee_asset ? m_fee_subaccount : m_subaccount;
                const auto utxo_subaccount = j_uint32ref(utxo, "subaccount");
                if (check_subaccount && utxo_subaccount != *check_subaccount) {
                    throw user_error("\"utxos\" elements must be from the same subaccount");
                } else {
                    check_subaccount = utxo_subaccount;
                }
            }
        }
        ...
        if (!utxos.contains(policy_asset)) {
            // Add an initial fee UTXO
            utxos[policy_asset] = nlohmann::json::array_t();
            m_fee_subaccount = j_uint32ref(add_fee_utxo(m_details), "subaccount");
        }
        GDK_RUNTIME_ASSERT(m_fee_subaccount.has_value());
    ...
    nlohmann::json& create_redeposit_transaction_call::add_fee_utxo(nlohmann::json& to)
    {
