...
#define SIG_LOW SIG_BYTES(OP_SUBSTR, OP_SUBSTR)
    ...
    std::string get_address_from_utxo(session_impl& session, const nlohmann::json& utxo, bool verify_script)
    {
        ...
        if (addr_type == p2sh_p2wpkh || addr_type == p2wpkh || addr_type == p2pkh || addr_type == p2tr) {
            ...
            if (addr_type == p2pkh) {
                return base58_address(net_params.btc_version(), user_key.get_public_key());
            }
            std::vector<unsigned char> witness_program;
            if (addr_type == p2tr) {
                const auto xonly = user_key.get_tweaked_xonly_key(net_params.is_liquid());
                witness_program = scriptpubkey_p2tr_from_xonly_key(xonly);
            } else {
                witness_program = witness_script(user_key.get_public_key(), WALLY_SCRIPT_HASH160);
            }
